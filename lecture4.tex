...
The Sheaf of Rings on a given topological space is called \emph{The Structure Sheaf}.
...
We recall that the underlying set (`points') of $\Spec{R}$ is the prime ideals of $\C[x,y]$ (consisting of $(0)$, irreducible polynomials (see Hartshorne pg 7, Krull's \emph{Hauptidealsatz}), and the maximal ideals $(x-a,y-b)$).
...
As $(x-a,y-b)$ is a maximal ideal, this gives us a field, and there's a canonical isomorphism of $\C$-algebras to this ring.
...
\begin{remark}
    $\Spec{\Z}$ is a really funky place to do geometry: $\Spec{\Z}$ describes a non-singular curve.
\end{remark}
...
\begin{proof}
    ...
    Note that $\mathfrak{m}$ is explicitly maximal, so $P$ is either $\mathfrak{m}$ or $\Spec{R}$.
    But $\Spec{R}$ is not considered a prime ideal by our convention, so $V(\mathfrak{m})=\{\mathfrak{m}\}$.
\end{proof}

Quotienting by a maximal ideal of $\Spec{R}$ at a closed point gives us the base field of the scheme.
...
\subsection{Localisation}
    ...
    We will return to the examples of $S=\{1,f,f^2,f^3,\dots\}$ and $S$ the complement of a prime ideal (for example, $R=\Z$ and $S=(2)$, and the multiplicative subset is thus the odd numbers).
    ...
    Considering $S^{-1}R$ as the fractions with denominators coming from $S$ is fine when $R$ is an integral domain, but falls apart if it is not (although it is a useful notation).
    ...
    \subsection{Building the Structure Sheaf}
        ...
        \begin{enumerate}
            ...
            \item Given $f_i\in\mathcal{F}(U_i)$ for all $i$ such that for all $i,j$ the image of $f_i\in\mathcal{F}(U_i\cap U_j)$ equals the image of $j_j\in\mathcal{F}(U_i\cap U_j)$ then there exists $f\in\mathcal{F}(U)$ that maps to all the $f_i$.
        \end{enumerate}

        Let $\Spec{R}=D_f\cup D_g$, then there are maps $R\to R_g$ and $R\to R_f$.
